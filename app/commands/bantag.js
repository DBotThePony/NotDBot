...
module.exports = {
	...
	func: function(args, cmd, msg) {
		if (!DBot.IsPM(msg)) {
			...
			if (!realm)
				return 'Invalid realm. Valid are: server, client and channel' + Util.HighlightHelp(['bantag'], 2, args);
			
			if (!space)
				return 'Invalid space. Valid are: ' + DBot.ValidTagSpaces() + Util.HighlightHelp(['bantag'], 3, args);
			
			if (!firstTag)
				return 'You need at least one tag to ban' + Util.HighlightHelp(['bantag'], 4, args);
			...
			if (!space)
				return 'Invalid space. Valid are: ' + DBot.ValidTagSpaces() + Util.HighlightHelp(['bantag'], 2, args);
			
			if (!firstTag)
				return 'You need at least one tag to ban' + Util.HighlightHelp(['bantag'], 3, args);
