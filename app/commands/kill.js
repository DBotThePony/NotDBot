		...
		fs.stat(fpath, function(err, stat) {
			...
			MySQL.query('SELECT * FROM killicons WHERE "CLASSNAME" = ' + Util.escape(weapon) + ' OR "NAME" = ' + Util.escape(weapon) + ' OR "NAME" LIKE ' + Util.escape('%' + weapon + '%') + ' OR "CLASSNAME" LIKE ' + Util.escape('%' + weapon + '%'), function(err, data) {
				...
				if (username2) {
					...
					magikArgs.push('-fill', blu, '-draw', Util.MonospaceText(35 + calcWidthFirst + width, 50, username2, FontSize, FontSpacing));
				} else {
...
DBot.RegisterCommand({
	...
	func: function(args, cmd, msg) {
		...
		let sha = DBot.HashString(username + ' ' + weapon + ' ' + username2);
		let fpath = DBot.WebRoot + '/killfeed/' + sha + '_b.png';
		let fpathU = DBot.URLRoot + '/killfeed/' + sha + '_b.png';
		...
		fs.stat(fpath, function(err, stat) {
			...
			MySQL.query('SELECT * FROM killicons WHERE "CLASSNAME" = ' + Util.escape(weapon) + ' OR "NAME" = ' + Util.escape(weapon) + ' OR "NAME" LIKE ' + Util.escape('%' + weapon + '%') + ' OR "CLASSNAME" LIKE ' + Util.escape('%' + weapon + '%'), function(err, data) {
				...
				if (username2) {
					magikArgs.push('-draw', Util.MonospaceText(40, 50, username, FontSize, FontSpacing));
					magikArgs.push('-draw', 'image over ' + (calcWidthFirst + 10 + width / 2) + ',' + (height / 2 - iheight / 2) + ' 0,0 "./resource/killicons/' + data[0].FILENAME + '"');
					magikArgs.push('-fill', red, '-draw', Util.MonospaceText(35 + calcWidthFirst + width, 50, username2, FontSize, FontSpacing));
				} else {
					magikArgs.push('-draw', 'image over 40,' + (height / 2 - iheight / 2) + ' 0,0 "./resource/killicons/' + data[0].FILENAME + '"');
					magikArgs.push('-draw', Util.MonospaceText(60 + width, 50, username, FontSize, FontSpacing));
				}
		...
		fs.stat(fpath, function(err, stat) {
			...
			MySQL.query('SELECT * FROM killicons WHERE "CLASSNAME" = ' + Util.escape(weapon) + ' OR "NAME" = ' + Util.escape(weapon) + ' OR "NAME" LIKE ' + Util.escape('%' + weapon + '%') + ' OR "CLASSNAME" LIKE ' + Util.escape('%' + weapon + '%'), function(err, data) {
				...
				if (username2) {
					magikArgs.push('-draw', Util.MonospaceText(40, 50, username, FontSize, FontSpacing));
					magikArgs.push('-draw', 'image over ' + (calcWidthFirst + 10 + width / 2) + ',' + (height / 2 - iheight / 2) + ' 0,0 "./resource/killicons/' + data[0].FILENAME + '"');
					magikArgs.push('-draw', Util.MonospaceText(35 + calcWidthFirst + width, 50, username2, FontSize, FontSpacing));
				} else {
					magikArgs.push('-draw', 'image over 40,' + (height / 2 - iheight / 2) + ' 0,0 "./resource/killicons/' + data[0].FILENAME + '"');
					magikArgs.push('-draw', Util.MonospaceText(60 + width, 50, username, FontSize, FontSpacing));
				}
