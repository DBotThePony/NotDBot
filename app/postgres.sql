...
CREATE TABLE IF NOT EXISTS uname_logs (
	...
	"FIRSTUSED" INTEGER NOT NULL DEFAULT currtime(),
...
CREATE OR REPLACE FUNCTION give_emoji_simple(giver INTEGER, receiver INTEGER, server INTEGER, emoji INTEGER)
RETURNS INTEGER AS $$
DECLARE myid INTEGER;
BEGIN
	INSERT INTO reactions_history ("GIVER", "RECEIVER", "SERVER", "EMOJI") VALUES (giver, receiver, server, emoji);
	SELECT "COUNT" INTO myid FROM reactions WHERE "USER" = receiver AND "EMOJI" = emoji;
	RETURN myid;
END; $$ LANGUAGE plpgsql;
...
CREATE TABLE IF NOT EXISTS hangman_score (
	"ID" INTEGER NOT NULL REFERENCES users ("ID"),
	...
	"LENGTH_WIN" INTEGER NOT NULL DEFAULT 0,
	"LENGTH_ABORT" INTEGER NOT NULL DEFAULT 0,
	"LENGTH_DEF" INTEGER NOT NULL DEFAULT 0,
	"CHARS_SUGGESTED" INTEGER NOT NULL DEFAULT 0,
	"CHARS_HIT" INTEGER NOT NULL DEFAULT 0,
	"CHARS_MISS" INTEGER NOT NULL DEFAULT 0,
	PRIMARY KEY ("ID")
);
