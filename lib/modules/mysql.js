...
DBot.GetUserID = function(obj) {
	...
	if (!DBot.UsersIDs[id])
		throw new Error('Initialize user first (' + obj.name + ')');
...
DBot.GetChannelID = function(obj) {
	...
	if (!DBot.ChannelIDs[id])
		throw new Error('Initialize channel first (' + obj.name + ')');
...
DBot.GetServerID = function(obj) {
	...
	if (!DBot.ServersIDs[id])
		throw new Error('Initialize server first (' + obj.name + ')');
	...
	DBot.query('SELECT ID FROM user_id WHERE UID = "' + id + '"', function(err, data) {
		if (!data[0]) {
			DBot.query('INSERT INTO user_id (UID) VALUES("' + id + '")', function(err, data) {
				...
				MySQL.query('REPLACE INTO `user_names` (`ID`, `USERNAME`) VALUES (' + DBot.UsersIDs[id] + ', ' + MySQL.escape(utf8.encode(user.username)) + ')', function(err) {
					if (!err)
						return;
