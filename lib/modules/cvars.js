...
cvars.ErrorMessages[FCVAR_CHANNELONLY] = 'Variable accepts only valid server channels';
cvars.ErrorMessages[FCVAR_USERONLY] = 'Variable accepts only valid users';
cvars.ErrorMessages[FCVAR_SERVERONLY] = 'Variable accepts only servers';
...
class ConVar {
	constructor(data, object) {
		...
		MySQL.query('SELECT `VALUE` FROM `cvar_' + this.realm + '` WHERE `ID` = ' + this.id + ' AND `CVAR` = "' + this.name + '"', function(err, data) {
			if (!data || !data[0]) {
				MySQL.query('INSERT INTO `cvar_' + me.realm + '` (`ID`, `CVAR`, `VALUE`) VALUES (' + me.id + ', "' + me.name + '", ' + MySQL.escape(me.defValue) + ')');
			} else {
	...
	setValue(val) {
		...
		for (let i in this.flags) {
			...
			if (flag == FCVAR_PRINTABLEONLY) {
				...
				if (channelID2) {
					if (!DBot.FindChannel(channelID2))
						return [false, FCVAR_CHANNELONLY];
					
					val = DBot.GetChannelID(channelID2);
					continue;
				}
	...
	joinFlags() {
		...
		for (let i in this.flags) {
			concatFlags += ' ' + cvars.Strings[this.flags[i]];
		}
	...
	onValueChanged(oldVal, newVal) {
		for (let i in this.callbacks) {
			this.callbacks[i](oldVal, newVal);
		}
	}
}
...
class ServerVarSession {
	...
	cvarlist() {
		...
		for (let id in this.cvars) {
			let cvar = this.cvars[id];
			output += Util.AppendSpaces(id, 15) + ' = "' + Util.AppendSpaces((!cvar.haveFlag(FCVAR_PROTECTED) && cvar.getFormatedString() || '[REDACTED]') + '"', 10) + '; ' + cvar.joinFlags() + '\n';
		}
