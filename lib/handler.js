...
DBot.HandleMessage = function(msg, isPrivate) {
	...
	*/
	...
	if (ServerBans.isBanned(command)) {
		msg.reply('Command is banned on entrie server ;w;');
		return;
	}
	
	if (ChannelBans.isBanned(command)) {
	...
	if (cCommand.id != command && cCommand.name != command) {
		...
		if (ChannelBans.isBanned(cCommand.id)) {
			msg.reply('Command is banned on this channel ;w;');
			return;
		}
		
		if (ServerBans.isBanned(cCommand.name)) {
			msg.reply('Command is banned on entrie server ;w;');
			return;
		}
		
		if (ChannelBans.isBanned(cCommand.name)) {
			msg.reply('Command is banned on this channel ;w;');
			return;
		}
	}
