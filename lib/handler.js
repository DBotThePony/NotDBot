...
DBot.ExecuteCommand = function(cCommand, msg, parsedArgs, rawcmd, command, rawmessage) {
	...
	if (cCommand.id != 'more' && cCommand.id != 'retry') {
		if (cCommand.more) {
			DBot.__LastMoreCommand[msg.channel.id][msg.author.id] = [cCommand, parsedArgs, rawcmd, rawmessage || msg.content];
		} else {
			DBot.__LastMoreCommand[msg.channel.id][msg.author.id] = undefined;
		}
		
		DBot.__LastRetryCommand[msg.channel.id][msg.author.id] = [cCommand, parsedArgs, rawcmd, rawmessage || msg.content];
	}
	
	hook.Run('PreExecuteCommand', cCommand.id, msg.author, parsedArgs, rawcmd, rawmessage || msg.content, msg);
	var reply = cCommand.func(parsedArgs, rawcmd, rawmessage || msg.content, msg);
	hook.Run('PostExecuteCommand', cCommand.id, msg.author, parsedArgs, rawcmd, rawmessage || msg.content, msg);
	hook.Run('CommandExecuted', cCommand.id, msg.author, parsedArgs, rawcmd, rawmessage || msg.content, msg);
