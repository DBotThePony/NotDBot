...
DBot.ExecuteCommand = function(cCommand, msg, parsedArgs, rawcmd, command, rawmessage, extraArgument, parsedHandlers) {
	...
	if (cCommand.id != 'more' && cCommand.id != 'retry') {
		if (cCommand.more) {
			DBot.__LastMoreCommand[msg.channel.id][msg.author.id] = [cCommand, parsedArgs, rawcmd, rawmessage || msg.content, extraArgument, parsedHandlers];
		}
		
		DBot.__LastRetryCommand[msg.channel.id][msg.author.id] = [cCommand, parsedArgs, rawcmd, rawmessage || msg.content, extraArgument, parsedHandlers];
	}
	
	hook.Run('PreExecuteCommand', cCommand.id, msg.author, parsedArgs, rawcmd, rawmessage || msg.content, msg, extraArgument, parsedHandlers);
	
	msg.oldReply = msg.reply;
	...
	msg.reply = function(str) {
		if (cCommand.id != 'more' && cCommand.id != 'retry' && parsedHandlers[0]) {
			...
			if (pipe) {
				...
				spliced.splice(0, 1);
				let splitted = DBot.TrimArray(Util.AppendArrays(spliced, str.split(' ')));
				...
				try {
					reply = pipe.func(parsedArgs, redoneCmd, msg);
				} catch(err) {
				...
				if (reply === true) {
					this.reply = this.oldReply;
					return;
				} else if (typeof reply == 'string') {
					this.oldReply(reply);
					return;
				} else if (reply === false) {
					this.oldReply('Pipe that you specified does not accept command output');
					return;
				}
			}
		}
		
		return this.oldReply(str);
	}
	...
	hook.Run('PostExecuteCommand', cCommand.id, msg.author, parsedArgs, rawcmd, rawmessage || msg.content, msg, extraArgument, parsedHandlers);
	hook.Run('CommandExecuted', cCommand.id, msg.author, parsedArgs, rawcmd, rawmessage || msg.content, msg, extraArgument, parsedHandlers);
...
DBot.HandleMessage = function(msg, isPrivate) {
	...
	*/
	...
	DBot.ExecuteCommand(cCommand, msg, parsedArgs, redoneCmd, command, null, null, parsedHandlers);
}
