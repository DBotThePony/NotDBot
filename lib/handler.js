...
DBot.HandleMessage = function(msg, isPrivate) {
	...
	*/
	...
	for (i = 2 - shift; i < splitted.length; i++) {
		if (splitted[i] == '')
			continue;
	...
	if (DBot.CommandsAntiSpam[msg.author.id]) {
		...
		console.log(delta);
		if (delta > 0) {
			msg.reply(':broken_heart: P... please, stop spamming with that command. Wait ' + (Math.floor(delta / 100)) / 10 + ' seconds');
			return;
		}
	}
	...
	if ((!parsedArgs || !parsedArgs[0]) && cCommand.argNeeded) {
		msg.reply(cCommand.failMessage || 'Invalid arguments');
		return;
	}
