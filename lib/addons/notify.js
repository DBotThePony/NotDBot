...
hook.Add('HumanJoinsServer', 'Notifications', function(member, server) {
	...
	var format = cvars.Server(server).getVar('notify_join')
...
hook.Add('HumanLeftServer', 'Notifications', function(member, server) {
	if (!cvars.Server(server).getVar('notifications').getBool())
		return;
	...
	var format = cvars.Server(server).getVar('notify_left')
	.replace('%user%', '<@' + member.id + '>')
	.replace('%username%', member.user.username)
	.replace('%server%', '**' + server.name + '**');
