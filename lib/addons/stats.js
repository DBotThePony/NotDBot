...
DBot.RegisterCommand({
	...
	func: function(args, cmd, rawcmd, msg) {
		...
		var query = 'SELECT `user_id`.`UID` as `USERID`, `user_names`.`USERNAME` as `USERNAME`, `stats__uphrases_server`.`COUNT` as `COUNT` FROM `user_id`, `user_names`, `stats__uphrases_server` WHERE `stats__uphrases_server`.`UID` = `user_id`.`ID` AND `user_names`.`ID` = `user_id`.`ID` AND `stats__uphrases_server`.`USERVER` = ' + ID + ' ORDER BY `COUNT` DESC LIMIT 0, 10';
		...
		MySQL.query(query, function(err, data) {
			try {
				...
				if (args[0]) {
					for (var i in data) {
						output += (Number(i) + 1) + '    <@' + data[i].USERID + '> --- ' + data[i].COUNT + ' phrases\n';
					}
				} else {
					for (var i in data) {
						output += (Number(i) + 1) + '    ' + utf8.decode(data[i].USERNAME) + ' --- ' + data[i].COUNT + ' phrases\n';
					}
				}
