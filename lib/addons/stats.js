		...
		MySQL.query(query, function(err, data) {
			try {
				...
				for (let i in data) {
					...
					MySQL.query('SELECT SUM(`stats__uwords_server`.`COUNT`) as `RESULT` FROM `stats__uwords_server` WHERE `stats__uwords_server`.`UID` = ' + data[i].ID + ' AND `stats__uwords_server`.`USERVER` = ' + ID, function(err, newData) {
						MySQL.query('SELECT COUNT(DISTINCT `stats__uwords_server`.`WORD`) as `RESULT` FROM `stats__uwords_server` WHERE `stats__uwords_server`.`UID` = ' + data[i].ID + ' AND `stats__uwords_server`.`USERVER` = ' + ID, function(err, newData2) {
							total--;
							words[data[i].ID] = newData[0].RESULT || 'WTF';
							uwords[data[i].ID] = newData2[0].RESULT;
		...
		MySQL.query(query, function(err, data) {
			try {
				...
				for (let i in data) {
					...
					MySQL.query('SELECT SUM(`stats__uwords_channel`.`COUNT`) as `RESULT` FROM `stats__uwords_channel` WHERE `stats__uwords_channel`.`UID` = ' + data[i].ID + ' AND `stats__uwords_channel`.`CHANNEL` = ' + ID, function(err2, newData) {
						MySQL.query('SELECT COUNT(DISTINCT `stats__uwords_channel`.`WORD`) as `RESULT` FROM `stats__uwords_channel` WHERE `stats__uwords_channel`.`UID` = ' + data[i].ID + ' AND `stats__uwords_channel`.`CHANNEL` = ' + ID, function(err1, newData2) {
							total--;
							...
							try {
								words[data[i].ID] = newData && newData[0].RESULT || 'WTF';
								uwords[data[i].ID] = newData2 && newData2[0].RESULT;
							} catch(err) {
								console.error(err);
							}
