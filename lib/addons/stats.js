		...
		MySQL.query(query, function(err, data) {
			try {
				...
				var continueFunc = function() {
					...
					if (args[0]) {
						for (var i in data) {
							output += (Number(i) + 1) + '    <@' + data[i].USERID + '> --- ' + data[i].COUNT + ' phrases (' + words[data[i].ID] + ' total words said; ' + uwords[data[i].ID] + ' unique words)\n';
						}
					} else {
						for (var i in data) {
							output += (Number(i) + 1) + '    ' + utf8.decode(data[i].USERNAME) + ' --- ' + data[i].COUNT + ' phrases (' + words[data[i].ID] + ' total words said; ' + uwords[data[i].ID] + ' unique words)\n';
						}
					}
