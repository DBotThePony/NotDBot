...
module.exports = {
	...
	desc: 'Tries to parody meme generator. URL can be user with valid avatar. If you want top test with spaces, you should put test in single or double quotes.\nThere is no need to put bottom text in quotes, all arguments are concated into one bottom string.',
				...
				if (stat && stat.isFile()) {
					...
					magik.on('close', function(code) {
						if (code == 0 && output != '') {
							...
							var calc = Math.floor((width - 80) / topText.length);
							...
							if (bottomText) {
								...
								var calc = Math.floor((width - 80) / bottomText.length);
