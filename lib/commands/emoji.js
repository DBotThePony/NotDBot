...
var regExpObj = new RegExp('(' + regExp + '|<:[a-zA-Z_-]+:([0-9]+)>|\n)', 'gi');
var customEmoji = new RegExp('<:[a-zA-Z_-]+:([0-9]+)>', 'g');
...
module.exports = {
	...
	func: function(args, cmd, rawcmd, msg) {
		...
		var continueFunc = function() {
			...
			var imageLayers = [];
			var currentLayer = ['(', emojiCollection[0], '-resize', '512x512!', ')'];
			...
			for (var i in emojiCollection) {
				...
				if (emojiCollection[i] == '\n') {
					currentLayer = [];
					imageLayers.push(currentLayer);
				} else {
					currentLayer.push('(', '-resize', '512x512!', emojiCollection[i], ')');
				}
			}
		...
		for (var i in emojiMatch) {
			var subStr = emojiMatch[i];
			
			if (subStr.substr(0, 2) == '<:') {
				// Custom Emoji
				customEmojiAmount++;
				
				var MYID = emojiCollection.length;
				...
				emojiCollection.push('./resource/emoji/' + unicode + '.png');
			}
		}
