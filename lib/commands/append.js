			...
			if (stat) {
				...
				for (let i in args) {
					...
					if (typeof arg == 'object') {
						...
						if (!url) {
							msg.reply('User have no avatar' + Util.HighlightHelp(['vappend'], Number(i) + 2, args));
							return;
						}
					} else {
						url = arg;
					...
					if (!Util.HasValue(allowedExts, ext)) {
						msg.reply('One of arguments is invalid' + Util.HighlightHelp(['vappend'], Number(i) + 2, args));
						return;
					}
