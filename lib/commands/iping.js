	...
	func: function(args, cmd, msg) {
		...
		if (matchIP) {
			...
			if (ip.match(protocol))
				return DBot.CommandError('As i can rub address with my hoof, i see there are protocol', 'iping', args, 1);
			
			if (ip.match(address))
				return DBot.CommandError('As i can rub address with my hoof, i see there are path', 'iping', args, 1);
