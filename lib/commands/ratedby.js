...
module.exports = {
	...
	func: function(args, cmd, msg) {
		if (!args[0]) {
			return DBot.CommandError('Must specify top text', 'ratedby', args, 1);
		}
		
		if (!args[1]) {
			return DBot.CommandError('Must specify age', 'ratedby', args, 2);
		}
		
		if (!args[2]) {
			return DBot.CommandError('Must specify at least one string', 'ratedby', args, 3);
		}
		
		var age = Util.ToNumber(args[1]);
		
		if (!age) {
			return DBot.CommandError('Must be a valid number', 'ratedby', args, 2);
		}
		
		var ageLen = age.toString().length;
		...
		fs.stat(fpath, function(err, stat) {
			if (stat) {
				...
				var magikArgs = [
					...
					'-draw', 'text ' + (570 - ageLen * 6) + ',4 "' + args[1] + '+™"',
