	...
	func: function(args, cmd, msg) {
		if (typeof args[0] == 'object') {
			...
			MySQL.query('SELECT * FROM `uptime` WHERE `ID` = ' + uid, function(err, data) {
				MySQL.query('SELECT * FROM `uptime_bot`', function(err, bdata) {
					if (err || !data || !data[0]) {
						msg.reply('<internal pony error>');
						return;
					}
					
					var bTOTAL_ONLINE = bdata[0].AMOUNT;
					var bTOTAL_TIME = CurTime() - bdata[0].START;
					var bTOTAL_OFFLINE = bTOTAL_TIME - bTOTAL_ONLINE;
					...
					var TOTAL_TIME = CurTime() - STAMP - bTOTAL_OFFLINE;
