...
var UpdateFunc = function() {
	...
	for (var i in Users) {
		...
		if (curStatus != 'offline') {
			MySQL.query('UPDATE `lastonline` SET `LASTONLINE` = ' + ctime + ' WHERE `ID` = ' + DBot.GetUserID(Users[i]), function(err, data) {
				if (err) {
...
module.exports = {
	...
	func: function(args, cmd, rawcmd, msg) {
		...
		MySQL.query('SELECT `LASTONLINE` FROM `lastonline` WHERE `ID` = ' + uid, function(err, data) {
			...
			msg.reply('As i remember user <@' + args[0].username + '> was last online at\n`' + formated + ' UTC +0:00` (' + deltaStr + ' ago)');
		});
	}
}
