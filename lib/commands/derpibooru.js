		...
		if (!args[0]) {
			...
			searchFunc = function() {
				...
				getRandomImage(function(parse, myID) {
					...
					for (var i in split) {
						if (!msg.channel.name.match('nsfw') && (ClientTags.isBanned(split[i]) || ServerTags && ServerTags.isBanned(split[i]) || ChannelTags && ChannelTags.isBanned(split[i]))) {
							searchFunc();
							return;
						}
					}
			...
			for (var i in args) {
				...
				for (var bk in bannedChars) {
					...
					if (!msg.channel.name.match('nsfw') && (ClientTags.isBanned(str) || ServerTags && ServerTags.isBanned(str) || ChannelTags && ChannelTags.isBanned(str))) {
